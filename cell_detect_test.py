...
#Import picture
# img = cv2.imread(FILE)
# ihc_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
ihc_rgb = data.imread(FILE)
patches = patch_ex.extract_patches_2d(ihc_rgb,(512,512), 150)
ihc_rgb = patches[0]

# print(np.shape(ihc_rgb.shape))
#Stain space conversion
# ihc_hrd = separate_stains(ihc_rgb, hrd_from_rgb)
ihc_hrd = separate_stains(ihc_rgb, hrd_from_rgb)
# plt.imshow(ihc_hrd, cmap='gray')
# plt.show()
# '''DAB'''
# #Rescale signals
# #[:, :, 012 color]
