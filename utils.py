...
def make_folds():
    ...
    files = np.array([basename(f) for f in glob.glob("data/features_one_class/train/*.npy")])
    # files = np.array([basename(f) for f in glob.glob("data/features_norm/train/*.npy")])
    # files = np.array([basename(f) for f in glob.glob("data/features/train_overlap/*.npy")])
